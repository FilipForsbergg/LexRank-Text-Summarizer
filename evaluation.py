...
def evaluate_models_for_n(
    ...
    text_scores = {"rouge1": [], "rouge2": [], "rougeL": []}
    ...
    for i in range(n_articles):
        ...
        lex_summary = " ".join(lexrank.summarize(text=article, n_sentences=n_sentences))
        text_summary = " ".join(textrank.summarize(text=article, n_sentences=n_sentences))
        lead_summary = " ".join(lexrank.lead_k_base_line(text=article, k=n_sentences))

        # ROUGE
        lex_rouge = scorer.score(reference, lex_summary)
        text_rouge = scorer.score(reference, text_summary)
        lead_rouge = scorer.score(reference, lead_summary)

        for key in lex_scores.keys():
            lex_scores[key].append(lex_rouge[key].fmeasure)
            text_scores[key].append(text_rouge[key].fmeasure)
...
def sweep_n_values(
    ...
    for res in results:
        n = res["n_sentences"]
        for model_name in ["lexrank", "textrank", "lead"]:
            model_label = (
                "LexRank" if model_name == "lexrank" 
                else "TextRank" if model_name == "textrank"
                else "Lead-k"
                )
